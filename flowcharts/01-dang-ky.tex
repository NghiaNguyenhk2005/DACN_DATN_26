...
\begin{tikzpicture}[farm/chart]
    ...
    \node[farm/process] (register) at (0,-1.25) {Nhà cung cấp: đăng ký, khai báo định danh và thông tin trang trại};
    \node[farm/process] (otp) at (0,-2.75) {Hệ thống gửi OTP;\\nhà cung cấp nhập mã xác thực};
    ...
    \node[farm/process] (ekyc) at (0,-6.75) {Nhà cung cấp: xác minh danh tính điện tử (eKYC) --- ảnh giấy tờ định danh và ảnh chân dung};
    \node[farm/process] (evidence) at (0,-8.55) {Nhà cung cấp: tải chứng nhận (nếu có), mã số, cơ quan cấp và hạn dùng};
    ...
    \node[farm/processnarrow] (supplement) at (6,-12.45) {Hệ thống: thông báo lý do của bước không đạt; nhà cung cấp bổ sung};
...
\end{tikzpicture}
\end{adjustbox}
\caption{Lưu đồ quy trình đăng ký, xác thực nhà cung cấp và kiểm duyệt chứng nhận}
